...
#         if not self.spy:
#             if self in team or self.game.tries == 5:
#                 return True
...
class CountingBot(Bot):
    ...
    def select(self, players, count):
        ...
        # sort dict by the value, then take 'count - 1' players
        lowest = sorted(self.failed_missions_been_on.items(), key=lambda key_val: key_val[1])
        lowest = [key_val[0] for key_val in lowest if key_val[0] != self][:(count-1)]
        selected.extend(lowest)

        return selected
    
    def vote(self, team):
        if self.spy: return False

        # get a list of all players with the highest failed mission count
        highest = max(list(self.failed_missions_been_on.values()))
        highest_players = [self.failed_missions_been_on[player] == highest for player in self.failed_missions_been_on]

        # if any player in the team is one of the 2 highest players, reject the team
        if len(highest_players) == 2:
          if any(player in highest_players for player in team):
              return False     
