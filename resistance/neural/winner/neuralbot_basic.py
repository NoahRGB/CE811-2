...
import numpy as np
from loggerbot_new import LoggerBot # this assumes our loggerbot was in a file called loggerbot.py
...
model = keras.models.load_model("neural/models/loggerbot_new_classifier3.keras")
...
class NeuralBot(LoggerBot):
    ...
    def calc_player_probabilities_of_being_spy(self):
        ...
        for p in self.game.players:
            ...
            # input_vector = [p.index, p.name, self.game.tries, self.missions_been_on[p], self.failed_missions_been_on[p]]
            ...
            input_vector = [avg_suspicion_of_voted_up_missions, avg_suspicion_of_voted_down_missions, self.game.wins==2, self.was_on_first_failed_mission[p], self.game.turn, self.missions_been_on[p], self.failed_missions_been_on[p], self.upvoted_missions_they_are_on[p], self.upvoted_missions_they_are_off[p]]
            # input_vector = input_vector[2:] # remove the first 4 cosmetic details, as we did when training the neural network
