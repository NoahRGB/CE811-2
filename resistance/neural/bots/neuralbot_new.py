...
model = keras.models.load_model("neural/models/loggerbot_new_classifier2.keras")
...
class NeuralBot(LoggerBot):
    ...
    def calc_player_probabilities_of_being_spy(self):
        ...
        for p in self.game.players:
            ...
            input_vector = [p.index, p.name, self.game.tries, self.missions_been_on[p], self.game.wins==2,
                            self.was_on_first_failed_mission[p], self.game.turn, self.failed_missions_been_on[p], self.upvoted_missions_they_are_on[p]] + self.num_missions_voted_up_with_total_suspect_count[p] + \
                           self.num_missions_voted_down_with_total_suspect_count[p]
            input_vector = input_vector[3:] # remove the first 4 cosmetic details, as we did when training the neural network
