...
class LoggerBot(Bot):
    ...
    def onVoteComplete(self, votes):
        ...
        for p in self.game.players:
            ...
            self.training_feature_vectors[p].append([p.index, p.name, self.game.tries, ratio_missions_on_that_fail, self.voted_majority_count[p], 3 - self.game.losses, 3 - self.game.wins, self.was_on_first_failed_mission[p], self.game.turn, self.missions_been_on[p], self.failed_missions_been_on[p], self.upvoted_missions_they_are_on[p], self.upvoted_missions_they_are_off[p], avg_suspicion_of_voted_up_missions, avg_suspicion_of_voted_down_missions])
