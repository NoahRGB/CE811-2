...
df0 = df0[["PlayerName", "VotingBias", "RatioMissionsBeenOnThatFail","VotedMajorityCount", "LastChance", "LossesLeft", "WinsLeft", "OnFirstFailedMission", "GameTurn", "MissionsBeenOn","FailedMissionsBeenOn","UpvotedMissionsTheyAreOn", "UpvotedMissionsTheyAreOff", "AvgSuspicionOfVotedUpMissions", "AvgSuspicionOfVotedDownMissions","Spy"]]
...
for bot, df in bot_dfs.items():
    x_train = df.values[:,1:15].astype(np.float32) # get rid of turn / try / playerid / playername
    ...
    y_train=df.values[:,15].astype(np.int32) # target column (spy or not)
