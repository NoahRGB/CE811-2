...
df0 = pd.read_csv("LoggerBot_huge.log",names=["Turn","Try","PlayerID","PlayerName","MissionsBeenOn","FailedMissionsBeenOn","VotedUp0","VotedUp1","VotedUp2","VotedUp3","VotedUp4","VotedUp5","VotedDown0","VotedDown1","VotedDown2","VotedDown3","VotedDown4","VotedDown5","Spy"])

# bot_names = ["Trickerton", "Logicalton", "NeuralBot", "Simpleton", "LoggerBot", "Bounder"]
bot_names = ["Trickerton"]
bot_dfs = {}
for bot in bot_names:
  bot_dfs[bot] = df0.query(f"PlayerName=='{bot}'")
  bot_dfs[bot] = bot_dfs[bot].iloc[:10000]

for bot, df in bot_dfs.items():
    x_train = df.values[:,4:18].astype(np.float32) # get rid of turn / try / playerid / playername
    y_train=df.values[:,18].astype(np.int32) # target column (spy or not)
    ...
    layers = [
       layers.Dense(10, activation="tanh", input_shape=(num_inputs,)),
