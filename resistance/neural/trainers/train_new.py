...
df0 = pd.read_csv("neural/trainers/LoggerBot.log",names=["PlayerID","PlayerName","Tries","LastChance", "OnFirstFailedMission", "GameTurn", "MissionsBeenOn","FailedMissionsBeenOn","UpvotedMissionsTheyAreOn", "VotedUp0","VotedUp1","VotedUp2","VotedUp3","VotedUp4","VotedUp5","VotedDown0","VotedDown1","VotedDown2","VotedDown3","VotedDown4","VotedDown5","Spy"])
...
x_train = df.values[:,3:21].astype(np.float32) # This filters out only the columns we want to use as input vector for our NN.
...
y_train=df.values[:,21].astype(np.int32) # This is our target column.
...
accuracy_by_turn=[]
maximum_turn=df['GameTurn'].max()
accuracy_metric=tf.keras.metrics.Accuracy()
print("maximum_turn",maximum_turn)
for turn in range(1,maximum_turn+1):
    df_restricted=df.query('GameTurn>='+str(turn)) # Pull out just those rows of the training data corresponding to later turns in the game
    
    x=df_restricted.values[:,3:21].astype(np.float32)
    y=df_restricted.values[:,21].astype(np.int32)
    y_guess=model(x)
