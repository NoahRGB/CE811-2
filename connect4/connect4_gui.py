...
from minimax import minimax, static_evaluator, negamax, negamax_prune
...
# ontrollers=[Agents.MINIMAX,Agents.USER]
    ...
    while not game_over:
        ...
        if current_agent != Agents.USER and not game_over:                
            if current_agent == Agents.MINIMAX:
                ...
                # move_choice_2, minimax_score_2 = negamax(board, current_depth=0, max_depth=3, player=turn)
                # move_choice_3, minimax_score_3 = negamax_prune(board, current_depth=0, max_depth=3, player=turn, alpha=-math.inf, beta=math.inf)
                
                print(f"Minimax move choice: {move_choice} with score {minimax_score}")
                # print(f"Negamax move choice: {move_choice_2} with score {minimax_score_2}")
                # print(f"Negamax prune move choice: {move_choice_3} with score {minimax_score_3}")
